 ...
 */
export function useCurrentChecked<T = Element>(
    props: CheckedControlProps<T>,
    onGroupUpdated?: (checked: boolean) => void,
) {
    const { checked } = props;

    const prevChecked = usePrevious(checked);

    const [current, setCurrent] = useState<boolean | undefined>(checked);
    ...
    useEffect(() => {
        if (previous !== current) {
            ...
            if (checked === undefined && current === true) {
                onGroupUpdated?.(current);
            }
        }
    });
